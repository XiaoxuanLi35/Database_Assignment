...
INSERT INTO `concert_songs` VALUES (1,1,1),(2,2,1),(3,3,1),(4,4,1),(5,5,1);
/*!40000 ALTER TABLE `concert_songs` ENABLE KEYS */;
UNLOCK TABLES;

--
-- Temporary view structure for view `concert_ticket_summary`
--

DROP TABLE IF EXISTS `concert_ticket_summary`;
/*!50001 DROP VIEW IF EXISTS `concert_ticket_summary`*/;
SET @saved_cs_client     = @@character_set_client;
/*!50503 SET character_set_client = utf8mb4 */;
/*!50001 CREATE VIEW `concert_ticket_summary` AS SELECT 
 1 AS `concert_id`,
 1 AS `concert_title`,
 1 AS `total_tickets`*/;
...
--
-- Final view structure for view `concert_ticket_summary`
--
...
/*!50001 VIEW `concert_ticket_summary` AS select `modified_concerts_schema`.`concerts`.`concert_id` AS `concert_id`,`modified_concerts_schema`.`concerts`.`concert_title` AS `concert_title`,count(`modified_concerts_schema`.`tickets`.`ticket_id`) AS `total_tickets` from (`modified_concerts_schema`.`concerts` join `modified_concerts_schema`.`tickets` on((`modified_concerts_schema`.`concerts`.`concert_id` = `modified_concerts_schema`.`tickets`.`concert_id`))) group by `modified_concerts_schema`.`concerts`.`concert_id`,`modified_concerts_schema`.`concerts`.`concert_title` having (count(`modified_concerts_schema`.`tickets`.`ticket_id`) > 0) */;
...
-- Dump completed on 2024-11-20 16:12:48
